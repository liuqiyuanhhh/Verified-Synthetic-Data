...
def evaluate_cvae(model, device, num_classes=10):
    """
    ...
    return total_loss / num_samples, total_recon_loss / num_samples, total_kl / num_samples
...
def plot_model_samples(model, save_path=None, latent_dim=20, num_classes=10, per_class=8, device=None):
    ...
    imgs = model.decoder.decode(z, y_onehot).view(-1, 1, 28, 28).cpu()
...
def evaluate_and_log(model, work_dir: str, tag: str,
    ...
    total, recon, kld = evaluate_cvae(model, device=device)
    row = {
        "tag": tag,
        "fid": fid_score,
        "precision": precision,
    ...
    with open(csv_path, "a", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["tag", "fid", "precision", "recall", "total_loss", "reconstruction_loss", "kl_divergence"])
        if write_header:
            writer.writeheader()
        writer.writerow(row)

    print(f"[EVAL] {tag}: FID={fid_score:.3f} | Precision={precision:.4f} | Recall={recall:.4f}| Total Loss={total:.4f} | Recon Loss={recon:.4f} | KL Div={kld:.4f}")
