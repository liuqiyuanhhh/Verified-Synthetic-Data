...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        if verbose:
            print(f"Epoch [{epoch+1}/{epochs}], Train Loss: {avg_loss:.4f}, Train BCE: {avg_bce:.4f}, Train KLD: {avg_kld:.4f}")
...
def train_model_with_validation(
    ...
    for epoch in range(epochs):
        ...
        avg_train_bce = train_bce / train_samples if train_samples > 0 else float('inf')
        avg_train_kld = train_kld / train_samples if train_samples > 0 else float('inf')
        ...
        avg_val_bce = val_bce / val_samples if val_samples > 0 else float('inf')
        avg_val_kld = val_kld / val_samples if val_samples > 0 else float('inf')
        # Record history
        ...
        if verbose:
            print(f"Epoch [{epoch+1}/{epochs}], Train Loss: {avg_train_loss:.4f}, BCE: {avg_train_bce:.4f}, KLD: {avg_train_kld:.4f}, Val Loss: {avg_val_loss:.4f}, BCE: {avg_val_bce:.4f}, KLD: {avg_val_kld:.4f}")
...
def calculate_validation_loss(
    ...
    with torch.no_grad():
        for x, y in val_loader:
            ...
            val_bce += bce.item()
            val_kld += kld.item()
    return val_loss / len(val_loader.dataset), val_bce / len(val_loader.dataset), val_kld / len(val_loader.dataset)   
